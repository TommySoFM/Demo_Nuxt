<template>
  <div class="monster-container">
    ...
    <div
      class="monster-card"
      :class="{ 'monster-card__filter': position !== index && position !== null }"
      v-for="(monster, index) in  monsters"
      :key="`monster_${index}`"
      @mouseover="position = index"
      @mouseout="position = null"
    >
      <img class="monster-card__icon" :src="monster.icon" />
      <div class="monster-card__name">{{ monster.name }}</div>
      <div
        :class="[ monster.discounted != null ? 'monster-card__price-out' : 'monster-card__price']"
      >${{ monster.price }}</div>
      <div
        class="monster-card__discounted"
        v-if="monster.discounted != null"
      >${{ monster.discounted }}</div>
      <button
        class="monster-card__button"
